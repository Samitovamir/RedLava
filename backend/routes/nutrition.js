...
function prefsBrief(p) {
  ...
  if (p.fodmap) parts.push(
    'ВАЖНО — пользователь на диете Low-FODMAP (назначено врачом). Строго избегай продуктов с ВЫСОКИМ FODMAP: ' +
    'лук, чеснок (в т.ч. луковый/чесночный порошок), пшеница/рожь/ячмень (обычный хлеб и паста), бобовые, ' +
    'молоко и лактоза, яблоки, груши, мёд, грибы, цветная капуста, спаржа. ' +
    'Используй НИЗКИЕ FODMAP: рис, гречка, овёс, киноа, картофель, морковь, цукини, огурец, помидор, банан, ' +
    'клубника, безлактозные молочные и твёрдый сыр, курица, говядина, яйца, тофу, рыба. ' +
    'Для КАЖДОГО блюда заполни поле fodmap (low/mod/high) и короткое fodmapReason (из-за чего уровень).')
  return parts.join(' ')
}
      ...
      meals: {
        ...
        items: {
          ...
          properties: {
            ...
            fodmap: { type: 'string', enum: ['low', 'mod', 'high'], description: 'Уровень FODMAP блюда (только если пользователь на Low-FODMAP): low — безопасно, mod — умеренно, high — триггер' },
            fodmapReason: { type: 'string', description: 'Коротко, из-за каких ингредиентов такой уровень (напр. «чеснок, пшеница»); для low можно пусто' },
            parts: {
              type: 'array',
...
const INTAKE_TOOL = [{
  ...
  input_schema: {
    ...
    properties: {
      ...
      fodmap: { type: 'string', enum: ['low', 'mod', 'high'], description: 'Уровень FODMAP еды на фото: low — безопасно, mod — умеренно, high — триггер' },
      fodmapReason: { type: 'string', description: 'Коротко, из-за каких ингредиентов такой уровень FODMAP (напр. «чеснок, пшеница, сливки»)' },
      note: { type: 'string', description: 'Короткий комментарий: что непонятно или на что обратить внимание' }
    },
    required: ['kcal', 'fodmap']
  }
}]
...
const FOOD_PROMPT = (health) =>
  ...
  (health
    ? `Оцени поле health (целое 1–10) — насколько это блюдо уместно ИМЕННО для этого человека ПРЯМО СЕЙЧАС, ` +
      ...
      `белок и сложные углеводы — выше; при высоком стрессе/позднем часе тяжёлое — ниже; ` +
      `если человек на Low-FODMAP, а блюдо высокого FODMAP (fodmap=high) — заметно снизь пользу, это триггер для ЖКТ, ` +
      `а низкий FODMAP за это не штрафуй). В note кратко поясни оценку. `
    : '') +
  'Оцени уровень FODMAP этой еды: fodmap = low / mod / high, и коротко fodmapReason — из-за каких ингредиентов ' +
  '(высокий FODMAP: лук, чеснок, пшеница/хлеб/паста, бобовые, молоко/лактоза, яблоки, груши, мёд, грибы). ' +
