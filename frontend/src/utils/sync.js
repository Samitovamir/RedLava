// Синхронизация данных между устройствами ОДНОГО аккаунта.
// Модель: блоб на сервере в ячейке этого аккаунта (см. backend/userScope.js). pull при
// старте приложения (гидрируем localStorage ДО показа интерфейса), затем фоновый
// debounced push изменений + push при уходе со страницы. Last-write-wins.
// Device-local (темы/раскладка/язык) и кэши Whoop/Garmin НЕ синхронизируем — они
// либо про устройство, либо восстанавливаются с сервера сами.

import { isGuest } from '../api/authFetch.js'

const SYNC_KEYS = [
  'albert-events',            // расписание
  'albert-memory',            // память ассистента об этом человеке
  'albert-history',           // журнал действий
