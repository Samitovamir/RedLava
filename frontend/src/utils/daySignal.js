...
export function buildSignalData({ events = [], facts = [] } = {}) {
  ...
  // Питание: цель КБЖУ из профиля
  try {
    const profile = loadProfile()
    const tgt = computeTarget(profile)
    const goalLabel = (GOALS.find(g => g.key === profile.goal) || {}).label || profile.goal
    lines.push(`Питание: цель «${goalLabel}», ~${tgt.kcal} ккал/день (белок ${tgt.protein} г, жиры ${tgt.fat} г, углеводы ${tgt.carb} г). В дни тренировок цель растёт на реальный расход.`)
  } catch { /* ignore */ }
...
export const SIGNAL_CONTEXT =
  ...
  'Анализы: <ключевое отклонение → короткая рекомендация> — ЭТУ СТРОКУ ДАВАЙ ТОЛЬКО если есть отклонение или это уместно; если анализов нет/всё в норме — СТРОКУ ПРОПУСТИ\n' +
  'Питание: <цель ккал / акцент дня>\n' +
...
export function parseSignal(text) {
  ...
  const ROW_LABELS = ['стресс', 'впереди', 'спорт', 'анализы', 'питание', 'сегодня', 'сон', 'осталось', 'восстановление', 'восст']
