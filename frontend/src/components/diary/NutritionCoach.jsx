...
const COACH_CONTEXT =
  'Ты помощник по питанию пользователя (триатлет, следит за здоровьем и формой). По его данным дай РОВНО ' +
...
export default function NutritionCoach({ target, eaten = 0, remaining = 0, intake, selectedDay }) {
  ...
  const snapshot = [
    ...
    fodmapOn ? 'пользователь на диете Low-FODMAP (назначено врачом).' : '',
  ...
  const summary = useAiSummary({
    ...
    context: COACH_CONTEXT
      + (fodmapOn ? ' пользователь на Low-FODMAP: совет к приёму — только низкий FODMAP (без лука, чеснока, пшеницы, бобовых, молока); коротко упомяни это.' : '')
